...
MAIN_CLASS=com.qibaike.tns.server.app.ThriftNameServer

# set log config
LOG_FILE="$CONF_DIR/log4j.properties"

#jvm config
JAVA_OPTS="-javaagent:$AGENT_DIR/propertiesloadagent-0.0.1.jar=$CONF_DIR/core.properties -Xms512M -Xmx512M -XX:+PrintGCDetails -XX:+PrintGCTimeStamps -XX:+UseParallelOldGC -XX:+UseAdaptiveSizePolicy -Dhostname=$HOSTNAME -Dlog4j.configuration=file:$LOG_FILE"
...
start(){
    ...
    java -server ${JAVA_OPTS}  -cp "${CLASSPATH}" ${MAIN_CLASS}
